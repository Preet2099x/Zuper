...
function ForgotPassword() {
  ...
  const handleSendCode = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/auth/provider/forgot-password`, { email });
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/auth/provider/reset-password`, {
        email,
        code,
        newPassword,
      });
          ...
          {step === 1 ? (
            <form onSubmit={handleSendCode}>
              ...
              <button
                type="submit"
                disabled={loading}
                className="brutal-btn w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white py-4 text-lg disabled:opacity-50"
              >
